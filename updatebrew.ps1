...
$formulaPath = "$PSScriptRoot/Formula/zolam.rb"
...
foreach ($platform in $platforms) {
    $url = "https://github.com/$repo/releases/download/v$Version/zolam-$platform.tar.gz"
    $tempFile = Join-Path ([System.IO.Path]::GetTempPath()) "zolam-$platform.tar.gz"
    ...
    $formula = $formula -replace "(?<=zolam-$platform\.tar\.gz`"`n\s+sha256 `")SHA256", $hash
}
